...
REPO=os.environ.get('GITHUB_REPOSITORY','hrtechifyed/the-unsaid-yet-said'); TOKEN=os.environ.get('GITHUB_TOKEN',''); GEMINI=os.environ.get('GEMINI_API_KEY',''); MODEL=os.environ.get('GEMINI_MODEL','gemini-3.6-flash'); EVENT=os.environ.get('GITHUB_EVENT_PATH','')
...
def gemini(prompt):
 url=f'https://generativelanguage.googleapis.com/v1beta/models/{urllib.parse.quote(MODEL)}:generateContent'; payload={'contents':[{'parts':[{'text':prompt}]}],'generationConfig':{'temperature':0.35}}; req=urllib.request.Request(url,data=json.dumps(payload).encode(),method='POST'); req.add_header('Content-Type','application/json'); req.add_header('x-goog-api-key',GEMINI)
