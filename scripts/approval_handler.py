...
REPO=os.environ.get('GITHUB_REPOSITORY','hrtechifyed/the-unsaid-yet-said'); GITHUB_TOKEN=os.environ.get('GITHUB_TOKEN',''); GEMINI_API_KEY=os.environ.get('GEMINI_API_KEY',''); GEMINI_MODEL=os.environ.get('GEMINI_MODEL','gemini-3.6-flash'); EVENT_PATH=os.environ.get('GITHUB_EVENT_PATH','')
...
def gemini(prompt):
 ...
 url=f'https://generativelanguage.googleapis.com/v1beta/models/{urllib.parse.quote(GEMINI_MODEL)}:generateContent'; payload={'contents':[{'parts':[{'text':prompt}]}],'generationConfig':{'temperature':0.6}}; req=urllib.request.Request(url,data=json.dumps(payload).encode(),method='POST'); req.add_header('Content-Type','application/json'); req.add_header('x-goog-api-key',GEMINI_API_KEY)
...
def dispatch_research(issue_number):
 api(f'https://api.github.com/repos/{REPO}/actions/workflows/research-script.yml/dispatches','POST',{'ref':'main','inputs':{'issue_number':str(issue_number)}})
def main():
 event=json.loads(Path(EVENT_PATH).read_text()); issue=event.get('issue',{}); user_comment=(event.get('comment',{}).get('body') or '').strip(); n=issue.get('number'); title0=issue.get('title',''); body=issue.get('body') or ''
